...
export default function Modal({ cart, orderTotal, onClose }: ModalProps) {
  return createPortal(
    <div className="modal-overlay">
      <div className="modal-content">
        <div className="modal-title-info">
          ...
          <div className="modal-title-info-header">
            <div className="text-Preset-1">Order Confirmed</div>
            <div className="">We hope you enjoy your food!</div>
          </div>
        </div>
        <div className="modal-confirmed-items">
          {cart.map((orderItem) => {
            return (
                  ...
                  <div className="confirmed-item-info">
                    ...
                    <div>
                      ...
                      <div>
                        ...
                        <span className="confirmed-item-price text-Preset-4">
                          @ {numberFormatter(orderItem.price)}
                        </span>
                      </div>
                    </div>
                  </div>
                  <div className="confirmed-item-total-price text-Preset-3">
                    {numberFormatter(orderItem.quantity * orderItem.price)}
                  </div>
                </div>
                <hr />
              </Fragment>
          ...
          <div className="confirmed-order-total">
            ...
            <span className="text-Preset-2"> {numberFormatter(orderTotal)}</span>
          </div>
        </div>
        <button className="btn btn-primary text-Preset-3" onClick={onClose}>
          Start New Order
        </button>
      </div>
    </div>,
